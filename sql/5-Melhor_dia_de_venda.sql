...
WHERE 
    ...
    AND (MONTH(ti.data) = IFNULL({mes}, MONTH(ti.data)))  -- Se o mês for informado, filtra pelo mês; senão, ignora
    AND (YEAR(ti.data) = IFNULL({ano}, YEAR(ti.data)))    -- Se o ano for informado, filtra pelo ano; senão, ignora
GROUP BY 
    DAY(ti.data)
ORDER BY 
    Total_Vendas DESC;
