...
df = pd.read_csv('csvs/Warspear - Coins - Retiradas.csv', skiprows=4)
...
for index, row in df.iterrows():
    ...
    if row['Valor'] != '':
        ...
        # if row['Histórico'] == "Pagamento efetuado":
        ...
        cursor.execute(
            "INSERT IGNORE INTO fato_transacao (id_tempo, id_cliente, id_movimentacao, id_reentrada, valor, coins, pontos) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s)",
            (id_tempo, id_cliente, id_movimentacao, row['ID'], valor_final, coins, pontos)
        )
